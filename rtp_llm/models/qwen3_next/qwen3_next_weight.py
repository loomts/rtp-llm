...
class Qwen3NextBaseWeight(ModelDeployWeightInfo):
    ...
    def _should_fuse_shared_expert(self) -> bool:
        ...
        if not (
            self.ep_size == 1 and self.dp_size == 1 and self.ffn_tp_size == self.tp_size
        ):
            return False
...
class Qwen35MoeWeight(Qwen3NextBaseWeight):
    ...
    def _create_moe_expert_weights_stacked(
        ...
        overrides_w1, overrides_w2 = self._get_shared_expert_overrides(
            fuse_shared, stacked=True
        )
        return [
            MoeAtomicWeight(
                W.moe_w2,
