...
def _should_fuse_shared_expert(wi) -> bool:
    ...
    if mc.inter_size != mc.moe_inter_size:
        return False
    if not (wi.ep_size == 1 and wi.dp_size == 1 and wi.ffn_tp_size == wi.tp_size):
        return False
...
def _get_shared_expert_overrides(prefix, expert_num, fuse_shared, stacked=False):
    ...
    overrides_w2 = {
        n: [
            CkptWeightInfo(
                prefix + "layers.{i}.mlp.shared_expert.down_proj.weight", identity
            )
        ]
    }
    gate = CkptWeightInfo(
        prefix + "layers.{i}.mlp.shared_expert.gate_proj.weight", identity
    )
    up = CkptWeightInfo(
        prefix + "layers.{i}.mlp.shared_expert.up_proj.weight", identity
    )
...
class TestFusionConditions(unittest.TestCase):
    ...
    def test_ffn_tp_mismatch(self):
        self.assertFalse(
            _should_fuse_shared_expert(self._make_wi(ffn_tp_size=2, tp_size=4))
        )
    ...
    def test_inter_size_mismatch(self):
        self.assertFalse(
            _should_fuse_shared_expert(
                self._make_wi(inter_size=1024, moe_inter_size=512)
            )
        )
    ...
    def test_unsupported_quant_type(self):
        self.assertFalse(
            _should_fuse_shared_expert(
                self._make_wi(
                    quant_is_quant=True, quant_type_name="MXFp4QuarkQuantConfig"
                )
            )
        )
    ...
    def test_supported_quant_type(self):
        self.assertTrue(
            _should_fuse_shared_expert(
                self._make_wi(
                    quant_is_quant=True,
                    quant_type_name="Fp8PerChannelCompressedQuantConfig",
                )
            )
        )
...
class TestGetSharedExpertOverrides(unittest.TestCase):
    ...
    def test_stacked_order_gate_up(self):
        overrides_w1, _ = _get_shared_expert_overrides(
            "model.", 256, True, stacked=True
        )
    ...
    def test_no_fuse_returns_none(self):
        self.assertEqual(
            _get_shared_expert_overrides("model.", 256, False), (None, None)
        )
