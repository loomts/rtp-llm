...
void SpeculativeSampler::batchSample(SpeculativeSamplerOutput&           sample_output,
    ...
    // Pinned destination + non_blocking makes the D2H truly async; consumers
    // already synchronize transfer_done_event before reading when is_pinned().
    const auto accept_pin           = sample_output.accept_tokens.options().device(torch::kCPU).pinned_memory(true);
    sample_output.accept_tokens_cpu = torch::empty(sample_output.accept_tokens.sizes(), accept_pin);
    sample_output.accept_tokens_cpu.copy_(sample_output.accept_tokens, /*non_blocking=*/true);
    sample_output.accept_len_cpu =
        torch::empty(sample_output.accept_len.sizes(), accept_pin.dtype(sample_output.accept_len.dtype()));
    sample_output.accept_len_cpu.copy_(sample_output.accept_len, /*non_blocking=*/true);
