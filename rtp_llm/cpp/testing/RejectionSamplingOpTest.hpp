    ...
    ReferenceOutput referenceRejectionSampling(int                         batch_size,
        ...
        for (int batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
            ...
            for (int i = 0; i < num_speculative_tokens; ++i) {
                ...
                bool accept     = (do_sample[batch_idx] && u * p < q) || (!do_sample[batch_idx] && same_token);
                if (accept) {
                    output.token_ids[batch_idx * (num_speculative_tokens + 1) + i] = draft_id;
                } else {
                    pos = i;
                    if (!do_sample[batch_idx]) {
                        // Greedy verification already has the exact target token; no residual sampling.
            ...
            if (pos == num_speculative_tokens) {
                // All speculative tokens accepted: emit the target bonus token.
