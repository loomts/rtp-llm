...
_enable_quick_allreduce = "quick" in _rocm_allreduce_strategies
_enable_trtllm_allreduce = "trtllm" in _rocm_allreduce_strategies
_enable_aiter_custom_ar = "aiter" in _rocm_allreduce_strategies

if _is_rocm_runtime and _rocm_allreduce_strategies != {"none"}:
    logging.info(
        "ROCm AllReduce enabled tiers (fixed order: quick -> trtllm -> "
        "aiter -> symm_mem -> NCCL): %s",
...
def prepare_rocm_graph_communication(
    ...
    if _enable_quick_allreduce:
        try:
            ...
            quick_ar_manager.ensure_initialized(
                group=tp.process_group, device_id=descriptor.device_index
            )
        except Exception as exc:
    ...
    if _enable_aiter_custom_ar:
        try:
            ...
            aiter_ar_manager.ensure_initialized(
                group=tp.process_group, device_id=descriptor.device_index
            )
        except Exception as exc:
            logging.warning(
...
def shutdown_graph_comm() -> None:
    ...
    if _enable_quick_allreduce:
        try:
            ...
            logging.exception("Failed to close quick_allreduce")
    if _enable_aiter_custom_ar:
        try:
            from rtp_llm.models_py.modules.base.rocm.aiter_custom_allreduce import (
                aiter_ar_manager,
            ...
            logging.exception("Failed to close aiter custom allreduce")
    try:
