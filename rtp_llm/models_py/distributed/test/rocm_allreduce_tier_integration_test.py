...
def _worker_quick_allreduce(
    ...
    try:
        ...
        if rank == 0:
            print(
                f"  [quick_allreduce] quant={quant_type} hidden={hidden_size} rel_diff={rel_diff:.2e} ✓"
            )
...
def _worker_aiter_allreduce(
    ...
    try:
        ...
        rtol, atol = 1e-3, 1e-4
        assert (
            rel_diff < rtol or diff < atol
        ), f"[Rank {rank}] aiter allreduce mismatch: rel={rel_diff:.6e}, abs={diff:.6e}"
        if rank == 0:
            print(
                f"  [aiter_allreduce] hidden={hidden_size} dtype={dtype} rel_diff={rel_diff:.2e} ✓"
            )
...
def _worker_tier_dispatch(
    ...
    try:
        ...
        if rank == 0:
            print(
                f"  [tier_dispatch] strategy={strategy} hidden={hidden_size} rel_diff={rel_diff:.2e} ✓"
            )
